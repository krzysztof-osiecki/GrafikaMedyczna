		...
		return ae -> {
			try {
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					...
					int[][] magnitude = new int[read.getHeight()][read.getWidth()];
					Direction[][] direction = new Direction[read.getHeight()][read.getWidth()];
					BufferedImage bufferedImage =
							CannyEdgeDetector.performCannyDetection(read, Double.valueOf(sigma), Integer.valueOf(low),
									Integer.valueOf(high), magnitude, direction);
		...
		return ae -> {
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				try {
					...
					BufferedImage bufferedImage =
							LinesDetector.performLineDetection(image, Double.valueOf(sMaks), Integer.valueOf(sMaks), Integer.valueOf(sHigh));
